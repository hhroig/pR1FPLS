#' Sequential crossvalidation (different penalty for each component)
#'
...
#'
#' cv_fem <- cv_seq(X = X, Y = Y, penalty_vec = c(0.01, 10, 100),
  ...
  for (ncomp_i in 1:ncomp) {
    ...
    for (row_lambda in 1:length(penalty_grid)) {
      for (i in 1:num_folds) {
        ...
        MSE_lambda_fold[row_lambda, i] <- mean(as.numeric(Y_fold_test -
                                                            stats::predict(object = res_fpls,
                                                                           newdata = X_fold_test))^2)

      }# nested loop num_folds
    } # nested loop  row_lambda
