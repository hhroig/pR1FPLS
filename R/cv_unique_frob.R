#' Crossvalidation (same lambda for all components) using Frobenius norm metric.
#'
...
cv_unique_frob <- function(X,
  ...
  for (ncomp_i in 1:ncomp) {
    ...
    MSE_lambda_fold <- matrix( NA, nrow = length(penalty_grid), ncol =  num_folds)
    ...
    for (row_lambda in 1:length(penalty_grid)) {
      for (i in 1:num_folds) {
        ...
        MSE_lambda_fold[row_lambda, i] <- mean( as.numeric( Matrix::t(Yc_old) %*% Xc_old - v %*% Matrix::t(w) )^2 )
