...
predict.fpls_fem <-  function(object, newdata, ...){
  ...
  pred <- Xc %*% (object$coefficients) + (object$Y_mean)
...
predict.fpls_fda <-  function(object, newdata, ...){
  ...
  pred <- matrix(data = NA, nrow = nrow(newdata), ncol = ncol(object$coefficient_function))


  for (j in 1:ncol(pred)) {
    for (i in 1:nrow(pred)) {

      pred[i , j] <- num_int_1d(argvals = object$argvals,
                                f_obs = Xc[i, ] * object$coefficient_function[ , j]) +
        object$Y_mean
...
predict.fpls_tps <-  function(object, newdata, ...){
  ...
  pred <- as.matrix(stats::predict(object$mvpls_model,
                                   new_data_pls)[ , , object$ncomp]) + object$Y_mean
...
#' Predict new data for FPCR-FEM model
#'
#' @param object an \code{fpcr_fem} model.
#' @param newdata new data matrix.
#' @param ... further arguments.  Currently not used
#'
#' @return predicted values
...
#'                    stripped = FALSE )
#'
#' Y_pred <- predict(pc_res, X)
#' Y_hat <- fitted.values(pc_res)
#' plot(Y_pred, Y_hat)
predict.fpcr_fem <-  function(object, newdata, ...){

  Xc <- scale(newdata, center = object$X_mean, scale = FALSE)

  pred <-  Xc %*% object$R0 %*% object$coefficient_function + object$Y_mean
