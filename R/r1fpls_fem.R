...
r1fpls_fem <- function(X,
  ...
  AUX_inv <- Matrix::solve( Matrix::t(C) %*% R0  %*% W )

  W_star_tD <- R0 %*% W %*% AUX_inv %*% Matrix::t(D) # Xc %*% W_star_tD = Y_hat
  ...
  if (stripped) { # fast return (for CV)
    ...
    Beta_hat_star <- W %*% AUX_inv %*% Matrix::t(D)

    # Fitted values:
    Y_hat <- TT %*% Matrix::t(D) + Y_mean
    ...
    ret <- list(W = W,
                ...
                coefficients = W_star_tD,
                fitted.values = Y_hat,
                coefficient_function = as.matrix(Beta_hat_star),
                Y_mean = Y_mean,
                X_mean = X_mean,
                elapsed = tictoc::toc(quiet = !verbose)
    )
